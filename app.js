...
mongo.connect('mongodb://pssplnodechat.centralus.cloudapp.azure.com/mongochatform', { useUnifiedTopology: true }, function (err, db) { //for azure DB
  //mongo.connect('mongodb://127.0.0.1/mongochatform', function (err, db) {   //for run in local db
  ...
  io.sockets.on('connection', function (socket) {
    ...
    chats.find().limit(1000).sort({ _id: 1 }).toArray(function (err, res) {
      if (err) {
        throw err;
      }
      // Emit the messages
    ...
    socket.on('nameandmessage', function (data) {
      ...
      // Insert message
      chat.insert({ msg: msg, Name: Name, socketId: socketId }, function () {
